...
if __name__ == "__main__":
    ...
    strings = fibonacci(22)
    simple = simple_string(2000, 100)
    ...
    for i in xrange(len(strings)):
        ...
        try:
            print i
            res[len(strings[i])] = timeIt("tree.find_tandem_repeats(False)", 0)
    ...
    for i in xrange(len(simple)):
        print i
        tree = SuffixTree(simple[i])
        simres[len(simple[i])] = timeIt("tree.find_tandem_repeats(False)", 0)
    ...
    for i in xrange(len(x2)):
        x2[i] = x2[i]*math.log(x2[i])
    for i in xrange(len(x)):
        x[i] = x[i]*math.log(x[i])
    plt.scatter(x, res.values(), color = ['red'])
    plt.scatter(x2, simres.values()[1:])
    plt.show()
